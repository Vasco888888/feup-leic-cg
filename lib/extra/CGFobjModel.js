...
export class CGFobjModel extends CGFobject {
  ...
  loadData(objectData) {
    ...
    var normals = [];
    var textures = [];
    var texStride = 2; // Default to 2 (u,v)
    var unpacked = {};
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (VERTEX_RE.test(line)) {
        ...
        if (textures.length === 0) texStride = elements.length;
        ...
        */
        ...
        for (var j = 0, eleLen = elements.length; j < eleLen; j++) {
          ...
          if (elements[j] in unpacked.hashindices) {
            ...
            // vertex textures
            if (textures.length) {
              unpacked.textures.push(+textures[(vertex[1] - 1) * texStride + 0]);
              unpacked.textures.push(+textures[(vertex[1] - 1) * texStride + 1]);
            }
